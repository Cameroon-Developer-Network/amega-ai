...
class LLMConfig(BaseModel):
    ...
    max_length: int = Field(
        default=1000, gt=0, description="Maximum length of generated text in tokens."
    )
    top_p: float = Field(
        default=0.9,
        ge=0.0,
...
class BackendConfig(BaseModel):
    ...
    organization_id: Optional[str] = Field(
        default=None, description="Organization ID for the LLM service"
    )
    model_name: str = Field(description="Name of the model to use")
    timeout: int = Field(default=30, gt=0, description="Timeout for API requests in seconds")
...
class Settings(BaseSettings):
    ...
    LLM_CONFIG: LLMConfig = Field(
        default_factory=LLMConfig, description="LLM generation parameters"
    )
            ...
            config_data.update(
                {
                    ...
                    "APP_VERSION": yaml_data["app"].get(
                        "version", cls.model_fields["APP_VERSION"].default
                    ),

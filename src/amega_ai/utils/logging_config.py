...
class ColoredFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        record.timestamp = datetime.fromtimestamp(record.created).strftime("%Y-%m-%d %H:%M:%S.%f")[
            :-3
        ]
    ...
    def process(self, msg: str, kwargs: Dict[str, Any]) -> tuple[str, Dict[str, Any]]:
        ...
        if self.extra:
            ...
            if "extra_context" in kwargs["extra"]:
                kwargs["extra"]["extra_context"] = _sanitize_context(
...
def get_logger(
    component_name: str = "amega_ai", extra_context: Optional[Union[dict, str]] = None
) -> logging.Logger:
    """

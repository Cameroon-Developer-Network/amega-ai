...
def create_test_token(username: str) -> str:
    ...
    token = create_access_token(data={"sub": username, "role": user["role"]}, expires_delta=expires)
    return token
...
def test_rbac_admin_access():
    ...
    endpoints = [("/test/user", "user"), ("/test/moderator", "moderator"), ("/test/admin", "admin")]
...
def test_request_validation():
    ...
    try:
        response = client.post("/test/content", json={"content": large_content}, headers=headers)
    ...
    try:
        response = client.post(
            "/test/content", headers={"Content-Type": "text/plain", **headers}, content="invalid"
        )
    ...
    # Test valid request
    response = client.post("/test/content", json={"content": "valid"}, headers=headers)
    assert response.status_code == 200
